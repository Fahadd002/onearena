...
export default function FacilitiesPage() {
  ...
  useEffect(() => {
    setPlaceholder("Search facilities...");
  ...
  const { data: pageData, isLoading } = useQuery({
    queryKey: ["facilities-admin", search, sortField, sortDirection, pagination.page, pagination.pageSize],
    queryFn: async () => {
      ...
      }>(API_ENDPOINTS.marketplace.facilities, { params });
      return result.data ?? { data: [], meta: { page: 1, limit: 10, total: 0, totalPages: 0 } };
    },
  });

  const facilities = pageData?.data ?? [];
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const res = await httpClient.post<Facility>(API_ENDPOINTS.marketplace.facilities, { name });
      return res.data;
    },
    onSuccess: () => {
  ...
  return (
    <section className="dashboard-section">
      <div>
        <h1 className="dashboard-title">Facilities</h1>
      </div>
      ...
      {/* Table */}
      {isLoading ? (
        <TableLoadingState message="Loading facilities..." />
      ) : (
              ...
              <TableBody>
                {facilities.map((facility) => (
                  <TableRow key={facility.id}>
                    <TableCell>
                      {editing[facility.id] !== undefined ? (
                        <Input
                          value={editing[facility.id]}
                          onChange={(e) => setEditing((prev) => ({ ...prev, [facility.id]: e.target.value }))}
                          autoFocus
                        />
                      ) : (
                        ...
                        {editing[facility.id] !== undefined ? (
                          <>
                            <Button
                              size="sm"
                              variant="default"
                              onClick={() => updateMutation.mutate({ id: facility.id, value: editing[facility.id] })}
                            ...
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => setEditing((prev) => { const next = { ...prev }; delete next[facility.id]; return next; })}
                            >
                              Cancel
                            </Button>
                          </>
                          ...
                          <>
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => setEditing((prev) => ({ ...prev, [facility.id]: facility.name }))}
                            >
                              Edit
                            </Button>
                            <DeleteConfirmDialog
                              open={deleteTarget?.id === facility.id}
                              onOpenChange={(open) => !open && setDeleteTarget(null)}
                              title="Delete Facility"
                              description={`Are you sure you want to delete "${facility.name}"? This action cannot be undone.`}
                              onConfirm={() => deleteMutation.mutate(facility.id)}
                              isPending={deleteMutation.isPending}
                              trigger={
                                <Button size="sm" variant="destructive" onClick={() => setDeleteTarget(facility)}>
                  ...
                  <TableRow>
                    <TableCell colSpan={2}>
                      <TableEmptyState
                        ...
                        description={search ? "Try adjusting your search." : "Add a facility above to get started."}
                      />
                    </TableCell>
                  </TableRow>
                )}
